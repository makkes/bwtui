...
define install_go_tool
	$(Q) mkdir -p $(GOBIN)
	$(Q) CGO_ENABLED=0 go install -v $$(grep $1 $(GO_TOOLS_FILE))
endef
...
define install_tool
	$(if $(1), \
		$(Q) asdf plugin list | grep -E '^$(1)$$' &>/dev/null || asdf plugin add $(1), \
		$(Q) grep -Eo '^[^#]\S+' $(REPO_ROOT)/.tool-versions | xargs -I{} bash -ec 'asdf plugin list | grep -E '^{}$$' &>/dev/null || asdf plugin add {}' \
	)
	asdf install $1
endef
...
install-tools.go: install-tool.golang ; $(info $(M) installing all go tools)
	$(Q) cat $(GO_TOOLS_FILE) | xargs -L1 go install -v
endif
...
upgrade-tools: ; $(info $(M) upgrading all tools to latest available versions)
	$(Q) grep -Eo '^[^#]\S+' $(REPO_ROOT)/.tool-versions | xargs -I{} bash -ec 'asdf plugin list | grep -E '^{}$$' &>/dev/null || asdf plugin add {}'
	$(Q) grep -v '# FREEZE' $(REPO_ROOT)/.tool-versions | \
		grep -Eo '^[^#]\S+' | \
		xargs -I{} bash -ec '\
			export VERSION="$$( \
				asdf list all {} | \
